...
const currencySymbolMap = {
  ...
  'CNY': '¥', 'GHS': '₵', 'NGN': '₦', 'ZAR': 'R', 'INR': '₹', 'BRL': 'R$', 'RUB': '₽',
  'MXN': '$', 'KRW': '₩', 'SGD': 'S$', 'NZD': 'NZ$', 'TRY': '₺', 'HKD': 'HK$', 'SEK': 'kr',
  'NOK': 'kr', 'DKK': 'kr', 'PLN': 'zł', 'THB': '฿', 'IDR': 'Rp', 'PHP': '₱', 'MYR': 'RM',
  'PKR': '₨', 'ILS': '₪', 'AED': 'د.إ', 'SAR': '﷼', 'QAR': 'ر.ق', 'KWD': 'د.ك',
  'DOGE': 'Ð', 'USDT': '₮', 'TRX': 'TRX', 'BSV': 'BSV', 'ETC': 'ETC', 'QTUM': 'QTUM',
  'VET': 'VET', 'ZIL': 'ZIL', 'WAVES': 'WAVES'
};

function updateDropdownSymbols() {
  const from = document.getElementById('fromCurrency').value;
  const to = document.getElementById('toCurrency').value;
  document.getElementById('fromLangSymbol').textContent = currencySymbolMap[from] || '';
  document.getElementById('toLangSymbol').textContent = currencySymbolMap[to] || '';
}

document.getElementById('fromCurrency').addEventListener('change', updateDropdownSymbols);
document.getElementById('toCurrency').addEventListener('change', updateDropdownSymbols);

// Initialize on page load
window.addEventListener('DOMContentLoaded', updateDropdownSymbols);